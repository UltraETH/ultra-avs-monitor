...
mod bid_manager_tests {
    ...
    async fn test_prune_old_blocks() {
        ...
        assert!(bid_manager
            ...
            .is_some());
        assert_eq!(
            bid_manager.get_bids_for_block(U256::from(200)).await.len(),
            1
        );
        assert!(bid_manager
            .get_highest_bid_for_block(U256::from(200))
            .await
            .is_some());
        ...
        assert!(
            bid_manager
                ...
                .await
                .is_none(),
            "Highest bid for block 100 should be pruned"
        );

        assert_eq!(
            bid_manager.get_bids_for_block(U256::from(101)).await.len(),
            1,
            "Block 101 should NOT be pruned"
        );
        assert!(
            bid_manager
                ...
                .await
                .is_some(),
            "Highest bid for block 101 should NOT be pruned"
        );

        assert_eq!(
            bid_manager.get_bids_for_block(U256::from(200)).await.len(),
            1,
            "Block 200 should NOT be pruned"
        );
        assert!(
            bid_manager
                .get_highest_bid_for_block(U256::from(200))
                .await
                .is_some(),
            "Highest bid for block 200 should NOT be pruned"
        );

        assert_eq!(
            bid_manager.get_bids_for_block(U256::from(201)).await.len(),
            1,
            "Block 201 should NOT be pruned"
        );
        assert!(
            bid_manager
                .get_highest_bid_for_block(U256::from(201))
                .await
                .is_some(),
            "Highest bid for block 201 should NOT be pruned"
        );
        ...
        assert_eq!(
            bid_manager.get_bids_for_block(U256::from(101)).await.len(),
            1,
            "Block 101 should still exist after retention 0"
        );
        ...
        assert_eq!(
            bid_manager.get_bids_for_block(U256::from(101)).await.len(),
            1,
            "Block 101 should still exist with large retention"
        );

        // Prune making current block the threshold (e.g. current 101, retention 1, threshold 100)
        bid_manager
            .add_bids(vec![create_test_bid(50, 5, "e")])
            .await;
        bid_manager.prune_old_blocks(U256::from(101), 1).await; // Threshold = 100
        assert_eq!(
            bid_manager.get_bids_for_block(U256::from(50)).await.len(),
            0,
            "Block 50 should be pruned"
        );
    }
    ...
    async fn test_add_bids_and_subscriptions() {
        ...
        assert_eq!(
            bid_manager
                .get_highest_bid_for_block(U256::from(1))
                .await
                .unwrap()
                .value,
            bid2_block1.value
        ...
        assert_eq!(
            bid_manager
                .get_highest_bid_for_block(U256::from(2))
                .await
                .unwrap()
                .value,
            bid1_block2.value
