...
mod sqlite_writer_tests {
    ...
    #[tokio::test]
    async fn test_sqlite_writer_initialize_creates_tables_normalized() {
        // Renamed test
        ...
        let old_bid_traces_count: i64 = sqlx::query_scalar(
            "SELECT count(*) FROM sqlite_master WHERE type='table' AND name='bid_traces'",
        )
        .fetch_one(&pool)
        .await
        .expect("Query to sqlite_master for bid_traces should not fail");
        assert_eq!(
            old_bid_traces_count, 0,
            "Old 'bid_traces' table should not exist"
        );
        ...
        assert_eq!(
            old_delivered_payload_traces_count, 0,
            "Old 'delivered_payload_traces' table should not exist"
        );
    ...
    #[tokio::test]
    async fn test_write_bid_normalized() {
        ...
        assert_eq!(
            count_rows(&pool, "builders").await,
            1,
            "Should be 1 builder"
        );
        assert_eq!(
            count_rows(&pool, "proposers").await,
            1,
            "Should be 1 proposer"
        );
        ...
        // Verify builder data
        let builder_row =
            sqlx::query("SELECT builder_pubkey FROM builders WHERE builder_pubkey = ?")
        ...
        assert_eq!(
            fee_recipient_db,
            format!("{:?}", test_bid.proposer_fee_recipient)
        );
    ...
    async fn test_write_delivered_payload_normalized() {
        ...
        writer
            .write_delivered_payload(test_payload.clone())
            .await
            .unwrap();
        ...
        assert_eq!(
            count_rows(&pool, "builders").await,
            1,
            "Should be 1 builder from payload"
        );
        assert_eq!(
            count_rows(&pool, "proposers").await,
            1,
            "Should be 1 proposer from payload"
        );
        assert_eq!(
            count_rows(&pool, "blocks").await,
            1,
            "Should be 1 block from payload"
        );
        assert_eq!(
            count_rows(&pool, "delivered_payloads").await,
            1,
            "Should be 1 delivered_payload"
        );
        ...
        let builder_row =
            sqlx::query("SELECT builder_pubkey FROM builders WHERE builder_pubkey = ?")
                ...
                .unwrap();
        assert_eq!(
            builder_row.get::<String, _>("builder_pubkey"),
            test_payload.builder_pubkey
        );
        ...
        let proposer_row = sqlx::query("SELECT proposer_pubkey, fee_recipient_address FROM proposers WHERE proposer_pubkey = ?")
            ...
            .fetch_one(&pool).await.unwrap();
        assert_eq!(
            proposer_row.get::<String, _>("proposer_pubkey"),
            test_payload.proposer_pubkey
        );
        assert_eq!(
            proposer_row.get::<String, _>("fee_recipient_address"),
            format!("{:?}", test_payload.proposer_fee_recipient)
        );
        ...
        let block_row = sqlx::query("SELECT block_hash, slot, num_tx, timestamp, gas_limit, gas_used, timestamp_ms FROM blocks WHERE block_hash = ?")
            ...
            .fetch_one(&pool).await.unwrap();
        assert_eq!(
            block_row.get::<String, _>("block_hash"),
            test_payload.block_hash
        );
        assert_eq!(
            block_row.get::<String, _>("slot"),
            test_payload.slot.to_string()
        );
        assert_eq!(
            block_row.get::<String, _>("num_tx"),
            test_payload.num_tx.to_string()
        );
        assert_eq!(
            block_row.get::<String, _>("timestamp"),
            test_payload.timestamp.to_string()
        );
        ...
        let payload_row = sqlx::query("SELECT value FROM delivered_payloads LIMIT 1")
            ...
            .unwrap();
        assert_eq!(
            payload_row.get::<String, _>("value"),
            test_payload.value.to_string()
        );
    ...
    #[tokio::test]
    async fn test_batch_write_normalized() {
        // Renamed and refactored
        let dir = tempdir().unwrap();
        let db_path = dir
            .path()
        ...
        assert_eq!(
            count_rows(&pool, "bids").await,
            0,
            "Bids should not be flushed yet"
        );
        assert_eq!(
            count_rows(&pool, "blocks").await,
            0,
            "Blocks should not be created yet"
        );
        assert_eq!(
            count_rows(&pool, "builders").await,
            0,
            "Builders should not be created yet"
        );
        assert_eq!(
            count_rows(&pool, "proposers").await,
            0,
            "Proposers should not be created yet"
        );
        ...
        assert_eq!(
            count_rows(&pool, "bids").await,
            3,
            "Batch of 3 bids should be flushed"
        );
        assert_eq!(
            count_rows(&pool, "blocks").await,
            3,
            "3 blocks should be created"
        ); // Each bid is for a new block_hash
        assert_eq!(
            count_rows(&pool, "builders").await,
            2,
            "Should be 2 unique builders (builder_1, builder_2)"
        );
        assert_eq!(
            count_rows(&pool, "proposers").await,
            3,
            "Should be 3 unique proposers (one per block_num)"
        );
        ...
        assert_eq!(
            count_rows(&pool, "bids").await,
            3,
            "Bid 4 should be in batch, not flushed yet"
        );
        ...
        assert_eq!(
            count_rows(&pool, "bids").await,
            4,
            "All 4 bids should be flushed"
        );
        assert_eq!(
            count_rows(&pool, "blocks").await,
            4,
            "All 4 blocks should be created"
        );
        assert_eq!(
            count_rows(&pool, "builders").await,
            3,
            "Should be 3 unique builders (builder_1, builder_2, builder_3)"
        );
        assert_eq!(
            count_rows(&pool, "proposers").await,
            4,
            "Should be 4 unique proposers"
        );
    ...
    #[tokio::test]
    async fn test_auto_flush_normalized() {
        // Renamed and refactored
        let dir = tempdir().unwrap();
        let db_path = dir
            .path()
        ...
        assert_eq!(
            count_rows(&pool, "bids").await,
            1,
            "Bid should be auto-flushed"
        );
        assert_eq!(
            count_rows(&pool, "blocks").await,
            1,
            "Block should be created by auto-flush"
        );
        assert_eq!(
            count_rows(&pool, "builders").await,
            1,
            "Builder should be created by auto-flush"
        );
        assert_eq!(
            count_rows(&pool, "proposers").await,
            1,
            "Proposer should be created by auto-flush"
        );
        ...
        assert!(
            bid_row.is_some(),
            "Auto-flushed bid data not found or incorrect"
        );
    ...
    #[tokio::test]
    async fn test_duplicate_handling_normalized() {
        // Renamed and refactored
        let dir = tempdir().unwrap();
        let db_path = dir
            .path()
        ...
        assert_eq!(
            count_rows(&pool, "bids").await,
            1,
            "Exact duplicate bid should be ignored"
        );
        ...
        let bid_diff_value = BidTrace {
            value: U256::from(4001),
            ..bid_orig.clone()
        };
        writer
            .write_bid_trace(bid_diff_value.clone())
            .await
            .unwrap();
        ...
        assert_eq!(
            count_rows(&pool, "bids").await,
            2,
            "Bid with different value for same block/builder should be inserted"
        );
        ...
        let payload_orig = DeliveredPayloadTrace {
            block_hash: "block_hash_401_payload_dup".to_string(),
            ..payload_orig
        };

        writer
            .write_delivered_payload(payload_orig.clone())
            .await
            .unwrap();
        writer
            .write_delivered_payload(payload_orig.clone())
            .await
            .unwrap(); // Exact duplicate payload for same block

        assert_eq!(
            count_rows(&pool, "delivered_payloads").await,
            1,
            "Duplicate delivered payload for same block_id should be ignored"
        );
        ...
        writer
            .write_bid_trace(bid_for_upsert.clone())
            .await
            .unwrap();
        writer.flush_bid_traces().await.unwrap();

        assert_eq!(count_rows(&pool, "blocks").await, 3); // block 400, 401, 402
        let block_before_payload_row = sqlx::query(
            "SELECT gas_limit, gas_used, timestamp_ms, num_tx FROM blocks WHERE block_hash = ?",
        )
        .bind(&bid_for_upsert.block_hash)
        .fetch_one(&pool)
        .await
        .unwrap();
        assert_eq!(
            block_before_payload_row.get::<String, _>("gas_limit"),
            bid_for_upsert.gas_limit.to_string()
        );
        assert_eq!(
            block_before_payload_row.get::<String, _>("num_tx"),
            bid_for_upsert.num_tx.to_string()
        );
        ...
        writer
            .write_delivered_payload(payload_for_upsert.clone())
            .await
            .unwrap();
        ...
        assert_eq!(
            block_after_payload_row.get::<String, _>("gas_limit"),
            bid_for_upsert.gas_limit.to_string(),
            "gas_limit from bid should persist"
        );
        assert!(
            block_after_payload_row
                .get::<Option<String>, _>("gas_used")
                .is_some(),
            "gas_used from bid should persist"
        ); // Assuming it was set by bid
        assert!(
            block_after_payload_row
                .get::<Option<String>, _>("timestamp_ms")
                .is_some(),
            "timestamp_ms from bid should persist"
        );
        // num_tx should be updated from the payload's value if COALESCE logic is correct (excluded.num_tx)
        assert_eq!(
            block_after_payload_row.get::<String, _>("num_tx"),
            payload_for_upsert.num_tx.to_string(),
            "num_tx should be updated from payload"
        );
    ...
    #[tokio::test]
    async fn test_shutdown_flushes_normalized() {
        // Renamed and refactored
        let dir = tempdir().unwrap();
        let db_path = dir
            .path()
        ...
        assert_eq!(
            count_rows(&pool, "bids").await,
            0,
            "Bid should not be flushed yet"
        );
        assert_eq!(
            count_rows(&pool, "blocks").await,
            0,
            "Block should not be created yet"
        );
        ...
        assert_eq!(
            count_rows(&pool, "bids").await,
            1,
            "Bid should be flushed on shutdown"
        );
        assert_eq!(
            count_rows(&pool, "blocks").await,
            1,
            "Block should be created on shutdown flush"
        );
        assert_eq!(
            count_rows(&pool, "builders").await,
            1,
            "Builder should be created on shutdown flush"
        );
        assert_eq!(
            count_rows(&pool, "proposers").await,
            1,
            "Proposer should be created on shutdown flush"
        );
        ...
        assert!(
            bid_row.is_some(),
            "Shutdown-flushed bid data not found or incorrect"
        );
